...
RCT_EXPORT_METHOD(setCategory
    ...
    if (category) {
        ...
        if(forceOutputSpeaker && isActive) {

            [[AVAudioSession sharedInstance] setCategory:category withOptions:AVAudioSessionCategoryOptionDefaultToSpeaker error:nil];
            ...
            if (!success) {
                NSLog(@"!!!!!!!!!!!!! 2 AVAudioSession error overrideOutputAudioPort:%@",error);

                success = [[AVAudioSession  sharedInstance] setCategory:category withOptions:AVAudioSessionPortOverrideNone error:&error];

                if(!success) {
                    NSLog(@"!!!!!!!!!!!!! 2 Second override failed AVAudioSession error overrideOutputAudioPort:%@",error);
                }
            }
            else {
                NSLog(@"!!!!!!!!!!!!! 2 SUCCESS OVERRIDE SPEAKER");
            }
...
RCT_EXPORT_METHOD(forceOutputOnSpeaker : (BOOL)value) {
    forceOutputSpeaker = value;
}
